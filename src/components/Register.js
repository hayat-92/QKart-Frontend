...
class Register extends React.Component {
  ...
  performAPICall = async () => {
    ...
    try {
      ...
      var raw_response=await fetch(url, {
        method:"POST",
        body:JSON.stringify({
      ...
      var response=await raw_response.json();
    } catch (error) {
      flag=false;
    }
    ...
    if(this.validateResponse(flag, response)){
      return response;
    }
   ...
   */
  validateInput = () => {
    let username=this.state.username, password=this.state.password, confirmPassword=this.state.confirmPassword;
    let validate=true;
    if(username.length==0){
      message.error("Username is a required field!");
      validate=false;
    }else if(username.length<6 || username.length>32){
      message.error("Username length must be between 6 and 32 characters!")
      validate=false;
    }
    ...
    if(confirmPassword!==password){
      message.error("Confirm-password must be equal to Password!")
      validate=false;
    }
