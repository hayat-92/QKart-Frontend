...
export default class Cart extends React.Component {
  ...
  getQuantityElement = (item) => {
    if (this.props.checkout) {
      ...
      return <InputNumber min={1} max={10} defaultValue={item.qty} onChange={(e) => {
        this.pushToCart(item.product._id, e, false);
      }} />
    }
  ...
  render() {
    return (
      ...
      >
        ...
        {!(this.props.checkout) ? <Button style={{ "background": "orange" }} size="large" onClick={() => {
          if (this.state.items) {
            this.props.history.push("/checkout");
          } else {
            message.info("You must add items to cart first");
          }
        }} >{<ShoppingCartOutlined />}Checkout</Button> : ""}
