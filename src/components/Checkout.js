...
class Checkout extends React.Component {
  ...
  deleteAddress = async (addressId) => {
    ...
    if (this.validateResponse(errored, response, "delete")) {
      if (response) {
  ...
  checkout = async () => {
    ...
    if (this.validateResponse(errored, response, "checkout")) {
        ...
        message.success("Your order is successful!");
        // 2. Update user's balance in localStorage
        let remainbal= this.state.balance - this.cartRef.current.calculateTotal();
        localStorage.setItem("balance", remainbal);
   ...
   */
  order = async () => {
    var totalCost=this.cartRef.current.calculateTotal();
    if(this.state.balance < totalCost){
      message.error("You have insufficient amount in wallet!");
    }else if(!this.state.addresses && !this.state.addresses[this.state.selectedAddressIndex]){
      message.error("Plz add or select address!")
    }else{
      this.checkout();
    }
  ...
  async componentDidMount() {
    ...
    if(!this.state.products){
      ...
      message.error("Your cart is empty, Plz add item to cart!");
    }
      ...
      <>
        ...
        <div className="checkout-container">
          <Row>
            ...
            {/* Display the cart */}
            <Col xs={{ span: 24, order:1 }} lg={{ span:6, order:2}} className="checkout-cart">
              <div>
